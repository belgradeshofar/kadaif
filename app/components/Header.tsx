      ...
      <header className="relative bg-[#1f140b] border-b border-[#c79f61] shadow-md z-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
          ...
          <Link
            ...
            style={{ fontFamily: "Georgia, Times New Roman, Times, serif", fontWeight: 700, letterSpacing: '0.03em' }}
          >
            Kadaif Shop
          </Link>
          ...
          <nav className="hidden sm:flex space-x-4 md:space-x-8">
            {['Početna', 'Kategorije', 'Kontakt'].map((label, idx) => {
              const isButton = label === 'Kategorije';
              const common = "px-4 py-2 text-sm font-semibold uppercase rounded-full transition";
              const activeClasses = "text-[#f1e5d1] hover:bg-[#c79f61] hover:text-black";
              return isButton ? (
                <button
                  key={label}
                  onClick={scrollToCategories}
                  className={`${common} ${activeClasses}`}
